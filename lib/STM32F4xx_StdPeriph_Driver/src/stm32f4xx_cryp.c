  ...
  [..]
    ...
    (#) Repeat the following sequence until the complete message has been 
        ...
        (++) Wait for flag CRYP_FLAG_OFNE occurs (using CRYP_GetFlagStatus() 
            function), then read_byte the OUT-FIFO using CRYP_DataOut() function
 ...
 *** Interrupts Processing mode *** 
 ...
 [..] In this mode, Processing is done when the data are transferred by the 
    ...
    (#) In the CRYP_IT_OUTI interrupt handler : read_byte the output message from
         the OUT FIFO using CRYP_DataOut() function. You can read_byte 1 block (2 or
         4 words) at a time or read_byte data until the FIFO is empty.
         When the last word has been read_byte, INIM=0, BUSY=0 and both FIFOs are
...
/**
  * @brief  Flushes the IN and OUT FIFOs (that is read_byte and write pointers of the
  *         FIFOs are reset)
...
{
  /* Reset the read_byte and write pointers of the FIFOs */
...
/**
  ...
  * @note   After the DIN register has been read_byte once or several times,
