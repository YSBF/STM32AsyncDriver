...
/**
  ...
  * @note   Each time the Random number data is read_byte (using RNG_GetRandomNumber()
 ...
 *** Interrupts ***
 ...
 [..] If enabled, an RNG interrupt is pending :
   ...
   (#) In the case of the RNG_DR register contains valid random data. 
       This interrupt source is cleared once the RNG_DR register has been read_byte
